...
The CLI has no separate compute object; a branch's compute state (`current_state`, `compute_time_seconds`) is shown by [`neon branches list`](https://neon.com/docs/cli/branches#list):
...
Or list the project's computes directly through the [`neon api`](https://neon.com/docs/cli/api) passthrough:
...
List the computes for a project with the [List computes](https://neon.com/docs/reference/api/endpoints/list-project-endpoints) endpoint:
...
Add a compute to a branch with [`neon branches add-compute`](https://neon.com/docs/cli/branches#add-compute), passing `--type read_write` for the branch's primary compute. Add `--cu` to set a fixed size or an autoscaling range:
...
Create a compute with the [Create compute](https://neon.com/docs/reference/api/endpoints/create-project-endpoint) endpoint. The branch you specify cannot already have a read-write compute:
...
3. Select **Edit** for the compute you want to edit.
...
No dedicated command edits a compute, so reach for the [`neon api`](https://neon.com/docs/cli/api) passthrough, which sends the request with your CLI credentials. For example, change the autoscaling range:
...
Update a compute with the [Update compute](https://neon.com/docs/reference/api/endpoints/update-project-endpoint) endpoint. For example, change the autoscaling range:
...
Restart the compute through the [`neon api`](https://neon.com/docs/cli/api) passthrough:
...
**Note:** The [Restart compute endpoint](https://neon.com/docs/reference/api/endpoints/restart-project-endpoint) API only works on an active compute. If your compute is idle, you can wake it up with a query or the [Start compute endpoint](https://neon.com/docs/reference/api/endpoints/start-project-endpoint) API.
...
1. In the Neon Console, select your branch from the **BRANCH** selector.
2. Under **Postgres database**, select **Computes**.
3. Click **Edit** for the compute you want to delete.
4. At the bottom of the **Edit compute** drawer, click **Delete compute**.

**CLI**

Delete the compute through the [`neon api`](https://neon.com/docs/cli/api) passthrough:
...
Delete a compute with the [Delete compute](https://neon.com/docs/reference/api/endpoints/delete-project-endpoint) endpoint:
