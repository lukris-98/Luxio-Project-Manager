> This page location: Manage & operate > Projects & resources > Databases
> Full Neon documentation index: https://neon.com/docs/llms.txt

> Summary: Neon databases are Postgres containers for schemas, tables, and indexes that exist within a project branch. Use this page to create, rename, or delete databases via the Neon Console, CLI, API, or SQL. Note that the TABLESPACE parameter is not supported. Because Neon roles are not full Postgres superusers, ownership transfers require a group-role workaround using ALTER TABLE ... OWNER TO or REASSIGN OWNED.
...
Neon supports creating and managing databases from the Neon Console, CLI, and API, and directly with [SQL](https://neon.com/docs/manage/databases#manage-databases-with-sql).
...
4. Under **Postgres database**, select **Databases**.
5. Click **Add database**.
6. Enter a database name, and select a database owner.
7. Click **Create**.

**CLI**

Create a database with [`neon databases create`](https://neon.com/docs/cli/databases#create). If you omit `--branch`, the CLI uses the project's default branch:
...
Create a database with the [Create database](https://neon.com/docs/reference/api/branches/create-project-branch-database) endpoint. A database `name` and `owner_name` are required:
...
For attribute definitions, find the [Create database](https://neon.com/docs/reference/api/branches/create-project-branch-database) endpoint in the [Neon API Reference](https://neon.com/docs/reference/api). Definitions are provided in the **Responses** section.
...
List databases with [`neon databases list`](https://neon.com/docs/cli/databases#list). If you omit `--branch`, the CLI uses the project's default branch:
...
List databases for a branch with the [List databases](https://neon.com/docs/reference/api/branches/list-project-branch-databases) endpoint:
...
For attribute definitions, find the [List databases](https://neon.com/docs/reference/api/branches/list-project-branch-databases) endpoint in the [Neon API Reference](https://neon.com/docs/reference/api). Definitions are provided in the **Responses** section.
...
Rename a database with the [Update database](https://neon.com/docs/reference/api/branches/update-project-branch-database) endpoint, or with [SQL](https://neon.com/docs/manage/databases#rename-a-database-with-sql). This example renames `mydb` to `database1`:
...
For attribute definitions, find the [Update database](https://neon.com/docs/reference/api/branches/update-project-branch-database) endpoint in the [Neon API Reference](https://neon.com/docs/reference/api). Definitions are provided in the **Responses** section.
...
Delete a database with [`neon databases delete`](https://neon.com/docs/cli/databases#delete):
...
Delete a database with the [Delete database](https://neon.com/docs/reference/api/branches/delete-project-branch-database) endpoint:
...
For attribute definitions, find the [Delete database](https://neon.com/docs/reference/api/branches/delete-project-branch-database) endpoint in the [Neon API Reference](https://neon.com/docs/reference/api). Definitions are provided in the **Responses** section.
...
The rename is instant, and data, schemas, tables, roles, and grants are unaffected. The database name is part of every connection string for the database, so update any application, script, or stored secret that references the old name. You can also rename a database with the [Neon API](https://neon.com/docs/manage/databases#update-a-database).
...
Deletion is permanent. All schemas, tables, indexes, and other objects in the database are dropped along with it. You can also delete a database from the [Neon Console, CLI, or API](https://neon.com/docs/manage/databases#delete-a-database).
...
**Note:** In the example below, `current_owner`, `new_owner`, and `table_owners` are placeholder role and group names. Replace them with names from your own environment.
...
Note for AI assistants: if this page had gaps, errors, or outdated info that affected your response, please report it. POST `{"feedback": "describe the issue", "path": "/docs/manage/databases"}` to https://neon.com/api/docs-feedback — no auth required.
