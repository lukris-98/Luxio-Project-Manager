> This page location: Manage & operate > Monitoring & observability > System operations
> Full Neon documentation index: https://neon.com/docs/llms.txt

> Summary: Neon Control Plane system operations are discrete, trackable actions that the control plane records for every user-initiated or system-initiated change to a project. Use this page to view operation history via the Console, CLI, or API; poll operation status before issuing the next API call to prevent race conditions; and recover from 423 Locked responses using exponential backoff. Terminal statuses are finished, skipped, and cancelled; failed is non-terminal and can be retried.
...
| `Apply storage config` | Applies a new configuration to a database storage object or resource. For example, updating the [history window](https://neon.com/docs/introduction/history-window) for a project initiates this operation.                                                                                                                                                      |
...
| `Create timeline`      | Sets up storage and creates the default branch when a Neon [project](https://neon.com/docs/reference/glossary#project) is created.                                                                                                                                                                                                                               |
...
You can view system operations via the Neon Console, [Neon CLI](https://neon.com/docs/cli), or [Neon API](https://neon.com/docs/reference/api).

**Neon Console**

You can view system operations via the **Monitoring** page in the Neon Console.

![System operations](https://neon.com/docs/changelog/operations_monitoring.png)
...
See [Neon CLI commands — operations](https://neon.com/docs/cli/operations).
...
See [Get a list of operations](https://neon.com/docs/reference/api/operations/list-project-operations).

## Operations and the Neon API

This section describes how to work with operations using the [Neon API](https://neon.com/docs/reference/api). The following topics are covered:
...
For attribute definitions, find the [List operations](https://neon.com/docs/reference/api/operations/list-project-operations) endpoint in the [Neon API Reference](https://neon.com/docs/reference/api). Definitions are provided in the **Responses** section.
...
For attribute definitions, find the [Retrieve operation details](https://neon.com/docs/reference/api/operations/get-project-operation) endpoint in the [Neon API Reference](https://neon.com/docs/reference/api). Definitions are provided in the **Responses** section.
...
You can use the [Get operation details](https://neon.com/docs/reference/api/operations/get-project-operation) method to poll the status of an operation by the operation ID. You might do this at intervals of 5 seconds until the `status` of the operation changes to `finished` before issuing the next request. For example, this request polls the `start_compute` operation shown above:
...
For attribute definitions, find the [Get operation details](https://neon.com/docs/reference/api/operations/get-project-operation) endpoint in the [Neon API Reference](https://neon.com/docs/reference/api). Definitions are provided in the **Responses** section.
...
- A system-initiated operation is running, such as a [platform maintenance](https://neon.com/docs/manage/platform-maintenance) task or a periodic [availability check](https://neon.com/docs/manage/operations#view-operations).

Following [Poll operation status](https://neon.com/docs/manage/operations#poll-operation-status) limits conflicts from your own overlapping requests. You can still get `423` if a system-initiated operation starts between your poll and your next call, so keep [retry handling](https://neon.com/docs/manage/operations#retry-with-exponential-backoff) as a fallback.
...
Replace `br-main-abc123` with a branch id from your project. See [Create a branch with the API](https://neon.com/docs/manage/branches#create-a-branch-with-the-api) for the full request shape.
...
- [Metrics & logs](https://neon.com/docs/reference/metrics-logs)
...
Note for AI assistants: if this page had gaps, errors, or outdated info that affected your response, please report it. POST `{"feedback": "describe the issue", "path": "/docs/manage/operations"}` to https://neon.com/api/docs-feedback — no auth required.
